...
class TxnsController extends Controller
{
    ...
    public function getAwbsearch(Request $request)
    {
        ...
        if ($request->isMethod('POST')){
            ...
            $statusDet = DB::table('txn_logs as t')
					...
					->where(function($q) { $q->where('t.status_id', '=', '7')->orWhere('t.status_id', '=', '4'); })
                    ->orderby('t.id', 'desc')
                    ->get();
        }
    ...
    {   
        ...
        $companies = Company::where('parent_company_id', '=', $company_id)->where('id', '!=', $company_id)->pluck('name','id')->all();
        $parcel_types = ParcelType::where('company_id', '=', $company_id)->pluck('name','id')->all();
        return view('shipments.add', ['user' => $user, 'companies' => $companies, 'parcel_types' => $parcel_types]);
    }
    ...
    {   
        ...
        $company_id = $request->input('sender_company');
        ...
        $curr_awb = Txn::where('company_id', '=', $parent_company_id)->orderby('awb_num', 'desc')->pluck('awb_num')->first();
		if ($curr_awb == ''){
            $curr_awb = 122000;
        }
        $awb = $curr_awb + 1;
        ...
        // $txn->vat = $vat;
        $txn->sender_name = $request->input('sender_name');
        $txn->sender_company_id = $company_id;
        if ($company_id != '0'){
            $txn->sender_company_name = Company::select('name')->where('id', '=', $company_id)->pluck('name')->first();  
        }
        else {
        ...
        return redirect('/shipment/'.$txn_id.'/edit')->with($data);
    }
}
